<template>
      ...
      <v-row class="d-flex flex-column align-center justify-center">
        <v-col class="pa-0 ma-0">
          ...
          >
            ...
            >
                <h1 class="login-text mx-auto">Sign Up</h1>
                ...
                <small class="text-red" v-if="errorOccured">Error Occured: {{ errorType }}</small>
                <LAButton text="SIGN UP" class="w-100" margin="0px" @click="signupsubmit"/>
                <div class="alt-login-divider">
                    <div class="horizontal-bar"></div>
                    <div class="upper-case-text-format or-styling">or</div>
                    <div class="horizontal-bar"></div>
                </div>
  ...
  import useAuth from '~/compostables/useAuth'
  ...
  const signupsubmit = async () => {
    let response = await signUp(email.value, password.value) //.then(response =>{})
    if(!response.success){
  ...
  <style scoped>
    :root {
        --input-primary-color: rgb(31, 36, 40);    
        --input-secondary-color: rgb(11, 125, 224); 
    }
    ...
    .text-input-wrapper:focus-within {
        border-color: rgb(11, 125, 224) !important;
    }

    .text-input-wrapper:not(:focus-within) .cancel-email-input-button {
        display: none;
    }

    .text-input-wrapper {
        background: rgb(var(--color-polar));
        /* border: 2px solid rgb(var(--input-primary-color)); */
        border: 2px solid rgb(31, 36, 40);
        border-radius: 12px;
        display: flex;
        overflow: hidden;
        margin-top: 8px;
...
~/compostables/firebase~/compostables/useFirebase
